...
def get_ngram_filepath(language_iso, dataset):
    return f'{package_dir}/utils/charsplit/{dataset}-{language_iso}_ngram_probs.json'

def train_splitter(noun_tuples, language_iso, dataset, max_words: int = 10000000, max_len: int = 20) -> None:
    """
    ...
    # Write dicts to file
    with open(get_ngram_filepath(language_iso, dataset), "w") as f:
        json.dump({
            "prefix": start_ngrams,
            "infix": in_ngrams,
            "suffix": end_ngrams
...
class Splitter:
    def __init__(self, language_iso, dataset):
        with open(get_ngram_filepath(language_iso, dataset)) as f:
