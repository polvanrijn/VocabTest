...
def expand_word(noun_df, language_iso, key='lemma'):
    exp_key = key + '_expanded'
    noun_df = noun_df.iloc[[i for i, word in enumerate(noun_df[key]) if type(word) == str], :]
    is_cjk = language_iso in ['zh', 'ja', 'ko']
    primitives_to_character = {}
    if is_cjk:
        character2primitives = {}
        expanded_words = []
        for word in tqdm(noun_df[key], desc='Expanding ' + key):
            expanded_word = ''
        ...
        noun_df[exp_key] = expanded_words
        ...
        noun_df[exp_key] = noun_df[key]

    noun_df[exp_key + '_length'] = [len(word) for word in noun_df[exp_key]]
...
def compute_conditional_probabilities(noun_df, padding, key='lemma'):
    ...
    probabilities = []
    n_gram_list = []
    for word in tqdm(noun_df[exp_key], desc='Computing conditional probabilities'):
...
def is_compound_word(word, language_iso, lemma_set, splitter=None):
    """
    ...
    """
    from vocabtest.utils.charsplit import Splitter, train_splitter, get_ngram_filepath
    if splitter is None:
...
def create_block_set(language_iso):
    ...
    # Make sure all blockset entries are lowercase
